#![forbid(unsafe_code)]
//! Serving the four avenues from `canonical-api-server`.
//!
...
//! |---|------|---------------------|
//! | 1 | `direct_read` | none: the web server reads Postgres itself through `canonical-lib` |
...
///
/// Bind `CANONICAL_API_MTLS_ADDR` and pass the listener. One task per
...
/// Open the JetStream context for avenue 4 from `CANONICAL_NATS_URL`.
///
/// # Errors
