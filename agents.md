...
serve the reviewed change. Never force-push, use `git rebase` to rewrite shared
history, use `git stash` to conceal concurrent work, run destructive `git reset`
or `git clean`, delete branches/tags, expose credentials, or change repository
visibility or deployment state without an explicit reviewed task.

When synchronizing, inspect/fetch the remote first and integrate reviewed
changes with `git merge` or `git pull`, resolving conflicts semantically from
both sides. Preserve intended local work in explicit additive commits/branches
rather than stashing, resetting, cleaning, or force-pushing it away.
